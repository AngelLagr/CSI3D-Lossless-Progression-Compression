...
def print_initial_model(steps, output_file):
    # Collect vertices
    ...
    for v in vertices:
        for face in steps.get_faces(v):
            ...
            output_file.write("f " + " ".join(str(vertices_indices[v]) for v in face.vertices) + "\n")
        ...
        for i in reversed(range(1, len(steps))):
            ...
            for v in vertex_to_add: # vertices to update
                if v not in vertex_idx:
                    output_file.write(f"v {v.position[0]} {v.position[1]} {v.position[2]}\n")
