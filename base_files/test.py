...
class AllieeDesbrunEncoder:
    ...
    def __init__(self, model):
        ...
        self.layers = []  # List of (decimating_codes, cleaning_codes) per layer
    
    def encode(self, max_layers: int = 10) -> List:
        """
        Run the complete encoding process with progressive layers.
        
        From Section 3:
        The algorithm alternates between decimating and cleaning passes,
        creating progressive layers from coarse to fine detail.
        
        Each layer contains:
        - One decimating pass (removes vertices with valence 3-6)
        - One cleaning pass (removes vertices with valence 3)
        
        Args:
            max_layers: Maximum number of layer pairs to create
        ...
        for layer_idx in range(max_layers):
            print(f"\n=== Layer {layer_idx + 1}/{max_layers} ===")
            
            # Decimating pass
            print("  Running decimating pass...")
            decimating_codes = self._run_conquest_pass(
                min_valence=MIN_VALENCE_DECIMATION,
                max_valence=MAX_VALENCE_DECIMATION,
                pass_name=f"decimating-{layer_idx}"
            )
            
            # Cleaning pass
            print("  Running cleaning pass...")
            cleaning_codes = self._run_conquest_pass(
                min_valence=VALENCE_CLEANING,
                max_valence=VALENCE_CLEANING,
                pass_name=f"cleaning-{layer_idx}"
            )
            
            # Store layer information
            self.layers.append((decimating_codes, cleaning_codes))
            
            # If no codes were produced, we're done
            if not decimating_codes and not cleaning_codes:
                print(f"  ✓ No more vertices to process - stopping at layer {layer_idx + 1}")
                break
            
            print(f"  ✓ Layer {layer_idx + 1}: {len(decimating_codes)} decimating + {len(cleaning_codes)} cleaning codes")
            all_codes.extend(decimating_codes)
            all_codes.extend(cleaning_codes)
        
        self.connectivity_codes = all_codes
        
        print(f"\n{'='*60}")
        print(f"Encoding Summary:")
        print(f"  Total layers: {len(self.layers)}")
        print(f"  Total connectivity codes: {len(self.connectivity_codes)}")
    ...
    def write_obja(self, output_path: str):
        """
        Write the progressive mesh to an OBJA file with layered encoding.
        
        The file is structured with progressive layers:
        - Each layer contains a decimating pass followed by a cleaning pass
        - Stream markers (s) indicate the byte position for progressive loading
        ...
        with open(output_path, 'w') as f:
            ...
            # Write progressive layers with stream markers
            bytes_written = 0
            
            f.write(f'\n# ============================================\n')
            f.write(f'# Progressive encoding: {len(self.layers)} layers\n')
            f.write(f'# ============================================\n\n')
            
            for layer_idx, (decimating_codes, cleaning_codes) in enumerate(self.layers):
                f.write(f'\n# Layer {layer_idx + 1}\n')
                
                # Decimating pass codes
                if decimating_codes:
                    f.write(f'# --- Decimating pass: {len(decimating_codes)} operations ---\n')
                    for code in decimating_codes:
                        bytes_written += 1
                        f.write(f's {bytes_written}\n')
                        if code == 'N':
                            f.write(f'# Null patch\n')
                            # Visual marker (red color for null patches)
                            f.write('fc 1.0 0.0 0.0\n')
                        else:
                            f.write(f'# Vertex removal, valence {code}\n')
                            # Visual marker (green shades for successful removals)
                            intensity = min(1.0, code / 6.0)
                            f.write(f'fc 0.0 {intensity} 0.0\n')
                
                # Cleaning pass codes
                if cleaning_codes:
                    f.write(f'# --- Cleaning pass: {len(cleaning_codes)} operations ---\n')
                    for code in cleaning_codes:
                        bytes_written += 1
                        f.write(f's {bytes_written}\n')
                        if code == 'N':
                            f.write(f'# Null patch\n')
                            ...
                            f.write(f'# Vertex removal, valence {code}\n')
                            f.write(f'fc 0.0 0.0 1.0\n')  # Blue for cleaning pass
            
            f.write(f'\n# End of progressive encoding\n')
            f.write(f'# Total operations: {bytes_written}\n')
