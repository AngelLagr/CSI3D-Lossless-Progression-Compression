...
def test_retriangulate_valence5_orientation(left_tag, right_tag):
    ...
    V2 = Vertex((0.5, 1.5, 0.0))
    V1 = Vertex((0.0, 1.0, 0.0))
    C = Vertex((0.5, 0.5, 0.0))  # centre
    ...
    mesh.set_orientation((L,C),V1)
    mesh.set_orientation((V1,C),V2)
    mesh.set_orientation((C,V3),V2)
    mesh.set_orientation((R,V3),C)

    gate = Gate((L, R), C, mesh)
    patch_oriented = [L, R, V3, V2, V1]
    ...
    if (left_tag, right_tag) in [ (RetriangulationTag.Plus, RetriangulationTag.Minus),
                                 (RetriangulationTag.Minus, RetriangulationTag.Minus) ]:
        expected = V3
    elif (left_tag, right_tag) in [ (RetriangulationTag.Minus, RetriangulationTag.Plus)]:
        expected = V1
    else: 
        expected = V2
