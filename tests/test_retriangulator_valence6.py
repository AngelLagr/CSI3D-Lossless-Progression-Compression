...
def test_retriangulate_valence5_orientation(left_tag, right_tag):
    mesh = MeshTopology()

    L = Vertex((1.0, 0.0, 0.0))
    R = Vertex((2.0, 0.0, 0.0))
    V1 = Vertex((3.0, 1.0, 0.0))
    V2 = Vertex((2.0, 2.0, 0.0))
    V3 = Vertex((1.0, 2.0, 0.0))
    V4 = Vertex((0.0, 1.0, 0.0))
    C = Vertex((1.5, 1.0, 0.0))  # centre
    ...
    mesh.set_orientation((V1,C),R)
    mesh.set_orientation((V1,V2),C)
    mesh.set_orientation((V2,V3),C)
    mesh.set_orientation((V3,V4),C)
    mesh.set_orientation((C,V4),L)

    gate = Gate((L, R), C, mesh)
    patch_oriented = [L, R, V1, V2, V3, V4]
